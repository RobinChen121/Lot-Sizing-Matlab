...
%       optimalitytarget  is one of the characters
%                       c       for convex QP
...
try
   ...
   if nargin ~= 2
      display ('Usage: globalqpex1(filename,optimalitytarget)');
      display ('where');
      display ('filename        is a problem file with extension ');
      display ('                MPS, SAV, or LP (lower case is allowed)');
      display ('optimalitytarget  is one of the characters');
   ...
   elseif ( t == 'f' )
      cplex.Param.optimalitytarget.Cur = 2;
   elseif ( t == 'g' )
