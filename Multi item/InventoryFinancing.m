...
% multi item lot sizing problem with inventory financing
...
% pledge rate: beita;   inventory financing rate: r;    
% inventory financing lenth: ell

%% parameter values
B0 = 50000;     I10 = 0;      I20 = 0;    M = 1000000;
...
m1 = sdpvar(T,1);       m2 = sdpvar(T,1);


%% objective function
for t = 1 : T
    if t == 1
        I1 = I10 + y1(t) - d1(t) + w1(t) - m1(t);       I2 = I20 + y2(t) - d2(t) + w2(t) -m2(t);
        PI1 = I10 + y1(t) - d1(t) + w1(t) - m1(t);       PI2 = I20 + y2(t) - d2(t) + w2(t) -m2(t);
        if t <= L
            B = B0 - s1(t)*x1(t) - s2(t)*x2(t) - v1(t)*y1(t) - v2(t)*y2(t) - h1(t)*PI1(t) - h2(t)*PI2(t)-K(t);
        else
            B = B0 + p1(t-L)*(d1(t-L) - w1(t-L)) + p2(t-L)*(d2(t-L) - w2(t-L)) - s1(t)*x1(t) - s2(t)*x2(t) - v1(t)*y1(t) - v2(t)*y2(t) - h1(t)*PI1(t) - h2(t)*PI2(t)-K(t);
        end
    end
    
    if t > 1   
        I1 = [I1; I1(t-1) + y1(t) - d1(t) + w1(t)];       I2 = [I2; I2(t-1) + y2(t) - d2(t) + w2(t)];
        if t <= ell
            PI1 = [PI1; PI1(t-1) + y1(t) - d1(t) + w1(t) - m1(t)];       PI2 = [PI2; PI2(t-1) + y2(t) - d2(t) + w2(t) - m2(t)];
        else
            PI1 = [PI1; PI1(t-1) + y1(t) - d1(t) + w1(t) - m1(t) + m1(t-ell)];       PI2 = [PI2; PI2(t-1) + y2(t) - d2(t) + w2(t) - m2(t) + m2(t-ell)];
        end
        
        if t <= L
            B = [B; B(t-1)- s1(t)*x1(t) - s2(t)*x2(t) - v1(t)*y1(t) - v2(t)*y2(t) - h1(t)*PI1(t) - h2(t)*PI2(t)-K(T)]; 
        else
            B = [B; B(t-1) + p1(t-L)*(d1(t-L) - w1(t-L)) + p2(t-L)*(d2(t-L) - w2(t-L)) - s1(t)*x1(t) - s2(t)*x2(t) - v1(t)*y1(t) - v2(t)*y2(t) - h1(t)*PI1(t) - h2(t)*PI2(t)-K(t)]; 
        end 
    end
    
    
    if t <= ell
        B(t) = B(t) + beita*p1(t)*m1(t) + beita*p2(t)*m2(t);
    end
    if t>ell && t <= T-ell
        B(t) = B(t) + beita*p1(t)*m1(t) + beita*p2(t)*m2(t) - beita*p1(t-ell)*m1(t-ell)*(1+r)^ell - beita*p2(t-ell)*m2(t-ell)*(1+r)^ell;
    end
    if t > T-ell
        B(t) = B(t) - beita*p1(t-ell)*m1(t-ell)*(1+r)^ell - beita*p2(t-ell)*m2(t-ell)*(1+r)^ell;
    end
...
wC = [w1 >= 0; w2 >= 0];
yC = [y1 >= 0; y2 >= 0];
mC = [m1(1) <= I10; m1(2:T) <= PI1(1:T-1); m2(1) <= I20; m2(2:T) <= PI2(1:T-1); m1 >= 0; m2 >= 0; m1(T-ell+1:T) <=0; m2(T-ell+1:T) <=0];
constraints = [xyC; wdC; BC; IC; wC; yC; mC];
...
if sol.problem == 0
    ...
    fprintf('processable inventory level at each periods: PI1 = [');
    for i = 1:T
        fprintf('  %.2f ',value(PI1(i)));
    end
    fprintf(' ]\n');
    fprintf('processable inventory level at each periods: PI2 = [');
    for i = 1:T
        fprintf('  %.2f ',value(PI2(i)));
    end
    fprintf(' ]\n');
    ...
    fprintf(' ]\n');
     ...
     fprintf('pledged inventory: m1 = [');
    for i = 1:T
        fprintf('  %.2f ',value(m1(i)));
    end
    fprintf(' ]\n');
    fprintf('pledged inventory: m2 = [');
    for i = 1:T
        fprintf('  %.2f ',value(m2(i)));
    end
    fprintf(' ]\n');
