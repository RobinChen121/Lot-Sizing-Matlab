...
g1 =  sdpvar(T ,1);       g2 =  sdpvar(T ,1);  % 若没有 产品的参数取值，可能因为g2的存在得不到可行解
...
for t = 1 : T
    if t == 1
        ...
        B = B0 +p1(t)*g1(t)- s1(t)*x1(t) - s2(t)*x2(t) - v1(t)*y1(t) - v2(t)*y2(t) - h1(t)*I1(t) - h2(t)*I2(t)-K(t);
    end
    ...
    if t > 1
        ...
        if t <= L
            a= B(t-1) +p1(t)*g1(t) - s1(t)*x1(t) - s2(t)*x2(t) - v1(t)*y1(t) - v2(t)*y2(t) - h1(t)*I1(t) - h2(t)*I2(t)-K(t);
            B = [B; a];
        else
            a= B(t-1) + p1(t-L)*(d1(t-L) - w1(t-L)- g1(t-L)) - p1(t-L)*g1(t-L)*(1+ro)^L  + p2(t-L)*(d2(t-L) - w2(t-L))- s1(t)*x1(t) - s2(t)*x2(t) - v1(t)*y1(t) - v2(t)*y2(t) - h1(t)*I1(t) - h2(t)*I2(t)-K(t);
            B = [B; a];
        end
    end
...
yC = [y1 >= 0; y2 >= 0];
gC1 = [p1(1:T)*g1(1:T)<= C];
gC2 = [g1(1:T) <= d1(1:T)'-w1(1:T); g1(1:T) >= 0; g1(T-L+1:T) <=0];
constraints = [xyC; wdC; BC; IC; wC; yC; gC1; gC2];
...
if sol.problem == 0
    ...
    fprintf(' ]\n');
     ...
     fprintf('order loan quantity: g1 = [');
    for i = 1:T
        fprintf('  %.2f ',value(g1(i)));
    end
    fprintf(' ]\n');
