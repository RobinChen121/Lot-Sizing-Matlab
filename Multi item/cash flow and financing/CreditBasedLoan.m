function CreditBasedLoan
% multi item lot sizing problem with credit based loan
...
% loan amout: loan;    interest rate: r;   installment amount: installment;     loan payback interval: interval; 
...
T=length(p1);
d2=zeros(1,T);
s1 = 5000*ones(1,T);     s2 = 4000*ones(1,T);
v1 = 31*ones(1,T);     v2 = 25*ones(1,T);
h1 = 10*ones(1,T);      h2 = 10*ones(1,T);

L = 2;  alpha = 0.25;
K = 6000*ones(1,T);     Bmin = 0*ones(1,T);
loan = 5000;     r = 0.025;    
interval = 4;      installment = loan*(1+r)^T*interval/T;
...
for t = 1 : T
    if t == 1
        ...
        if t <= L
            B = B0 +loan - s1(t)*x1(t) - s2(t)*x2(t) - v1(t)*y1(t) - v2(t)*y2(t) - h1(t)*I1(t) - h2(t)*I2(t)-K(t);
        else
            B = B0 +loan + p1(t-L)*(d1(t-L) - w1(t-L)) + p2(t-L)*(d2(t-L) - w2(t-L)) - s1(t)*x1(t) - s2(t)*x2(t) - v1(t)*y1(t) - v2(t)*y2(t) - h1(t)*I1(t) - h2(t)*I2(t)-K(t);
        end
    end
